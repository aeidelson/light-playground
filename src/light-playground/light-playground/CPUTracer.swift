...
class CPUTracer: Tracer {
    required public init(completionQueue: DispatchQueue, simulationSize: CGSize) {
    ...
    func startAsync(layout: SimulationLayout, raysToTrace: Int, completion: (LightGrid) -> Void) {
        ...
        traceRays(layout: layout, producedSegments: &segments, maxSegments: raysToTrace)
    ...
    private func insideImageBounds(_ p: CGPoint) -> Bool {
        return (p.x >= 0) && (p.x < simulationSize.width) &&
            (p.y >= 0) && (p.y < simulationSize.height)
    }
    ...
    private func traceRays(
        ...
        while rayBuffer.count > 0 && segmentCount < maxSegments {
            ...
            guard insideImageBounds(ray.origin) else { continue }
